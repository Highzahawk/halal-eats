...
router.post(
  ...
  [
    ...
    body("restaurant_id").isUUID().withMessage("Valid restaurant_id (UUID) is required"),
    body("action").notEmpty().isString().withMessage("Action type is required"),
  ],
  async (req, res) => {
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const result = await pool.query(
        "UPDATE activity SET action = COALESCE($1, action) WHERE id = $2::uuid RETURNING *",
        [action, id]
      );
